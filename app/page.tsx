...
★ 특히 중요: 지침에 "링크", "URL", "주소" 관련 요청이 있으면
  - 참고 문서 원문에서 http 또는 https로 시작하는 URL을 모두 찾아내세요.
  - 찾은 URL을 각 recommendation의 "links" 배열에 { "label": "링크 설명", "url": "https://..." } 형식으로 포함하세요.
  - 링크가 없는 경우 "links": [] 로 두세요.
  - 지침에 링크 요청이 없더라도, 문서에서 관련 URL이 발견되면 해당 스킬 카드에 포함할 수 있습니다.

★ 지침이 ATL 추천 형식에 맞지 않는 자유 요청(예: "문서 요약", "링크 목록만", "표로 만들어줘")을 포함하는 경우:
  - 해당 내용을 "gemsExtra" 필드에 자유 형식 문자열로 작성하세요.
  - gemsExtra는 마크다운 사용 가능합니다.
...
[참고 문서 링크 추출 지침]
참고 문서에 포함된 URL(http:// 또는 https://로 시작하는 주소)을 빠짐없이 추출하여
가장 연관성 높은 recommendation의 "links" 배열에 넣어주세요.
링크가 전혀 없는 문서라면 links 배열은 비워두세요.

반드시 아래 JSON 형식으로만 응답하세요. 마크다운 코드블록 없이 순수 JSON만:
{
  "summary": "수업 분석 요약${gemsText.trim() ? ' (GEMS [summary] 규칙 적용)' : ' (2–3문장)'}",
  "usedFiles": ["참고한 파일명"],
  "gemsExtra": "${gemsText.trim() ? 'GEMS 지침 중 자유 형식 요청이 있으면 여기에 마크다운으로 작성. 없으면 빈 문자열.' : ''}",
  "recommendations": [
    {
      "category": "ATL 기능 범주명",
      "skill": "ATL 스킬명",
                        ...
                        {r.links.map((lk, li) => (
                          <a key={li} href={lk.url} target="_blank" rel="noreferrer"
                            style={{
                              ...
                              background: '#EEF4FF', border: '0.5px solid #AECBFA',
                              borderRadius: 'var(--radius-pill)', color: '#1A56DB',
